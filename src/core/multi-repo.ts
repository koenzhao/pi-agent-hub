...
import { renderWorktreeGuidance } from "./worktree-context.js";
...
export async function ensureMultiRepoWorkspace(session: ManagedSession, env: NodeJS.ProcessEnv = process.env): Promise<ManagedSession> {
  ...
  await writeWorkspaceContextFile(workspaceCwd, projectPaths, linkNames, renderWorktreeGuidance(session));
...
async function writeWorkspaceContextFile(workspaceCwd: string, projectPaths: string[], linkNames: string[], worktreeGuidance?: string): Promise<void> {
  ...
  await writeFile(join(workspaceCwd, "AGENTS.md"), renderWorkspaceContextFile(entries, worktreeGuidance), "utf8");
}
...
function renderWorkspaceContextFile(entries: Array<{ repoPath: string; linkName: string; contextPath: string; content: string }>, worktreeGuidance?: string): string {
  return [
    ...
    "Treat each linked repository as an independent project. Store repository-specific state, including `agent-work/`, inside that repository's workspace link—not at this workspace root.",
    ...(worktreeGuidance ? ["", worktreeGuidance] : []),
    "",

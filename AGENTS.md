...
- Active/Backlog groups and rows use `src/core/session-order.ts`: groups stay stable in `default`-first alphabetical order; inside each group, errors come first, then unacknowledged `waiting` rows newest-first, then `starting`/`running`, acknowledged `idle`, and stopped rows. Unacknowledged-waiting rows stay activity-driven newest-first; idle/acknowledged rows are persisted-order-driven, and `K`/`J` reorder freely within that tier. Group headers show a derived count of unacknowledged waiting rows without promoting parent state or subagent attention. Archived is globally newest-first by archive time and cannot be reordered. Do not sort by title or add a separate stopped section.
...
- Keep rendering pure/testable and ANSI width-safe through theme/layout helpers; reserve width for right-side badges/counts in left/right columns. In project rows, place an accented `⎇` before worktree titles and a dim `⧉ N` repository-count badge after multi-repo titles; keep full branch/count wording in details. In groups view, keep fixed Active/Backlog slots for a muted workflow stage and right-aligned activity age, with the age blank while running; Archived remains age-only. Archived collapse/disclosure stays ephemeral TUI state and must remain a synthetic non-session target so session actions cannot reach a stale real selection. Keep dashboard footer rendering in `src/tui/render-model.ts` distinct from managed-session tmux chrome in `src/core/tmux.ts`.

...
| Subagent rows | Automatic when `pi-tmux-subagents` reports them | See child work nested in groups view or expand a collapsed board tree with `Space`. |
| Workflow rail + board | Automatic when `workflow-runtime` reports its ordered steps; `v` toggles views | Supervise every Active session: canonical workflow trees in producer lanes, all others in `OTHER ACTIVE`, with grouped compact cards, running-subagent counts, plan context, and explicit attention. |
...
Pressing `v` switches to the read-only workflow board. Compatible Active workflow parent trees stay in producer-defined vertical lanes. Every remaining Active tree—without workflow metadata or on an incompatible pipeline—appears afterward in synthetic `OTHER ACTIVE`, which deliberately implies no workflow position. Backlog/Archived parents remain summarized in the footer. If multiple pipeline versions are visible, the most prevalent ordered-id sequence wins deterministically and the newest compatible labels/short codes supply the vocabulary.

Every lane and `OTHER ACTIVE` nests cards under the existing top-level parent group, with parent-card counts on lane and group headings. Unselected cards remain one row. Board subagent trees start collapsed: parents with descendants show `▸`, `Space` expands them to `▾`, and revealed children remain directly nested and independently navigable. Collapsing a selected child returns selection to its top-level parent. Active filters temporarily reveal matching ancestor/descendant context without changing expansion state. Top-level parent rows/cards add an independent `⚙︎N` when descendants are `starting` or `running`; waiting, idle, error, and stopped descendants do not count. Disclosure and the badge add no persistent state or parent-state promotion. Groups view remains expanded, and `K`/`J` reordering remains groups-view only.
...
Only the selected board card expands. Its board-only parent title prefers the authored `plan.feature` and falls back to the stored Hub title; groups view, details, tmux, registry naming, and subagent `agentName` are unchanged. The accent border encloses at most one line each for explicit attention, deterministic plan progress, and a distinct `→` action. During the producer's base `execute` step, plan progress is emphasized as `Phase X/Y · A/B tasks`; for all producer steps other than base `execute`, and for workflowless cards, it remains available as a dim `plan X/Y · A/B tasks` line. Focus mode does not change this distinction: it is a display mode over Execute. Running state changes the runtime glyph and suppresses attention only; valid deterministic phase/task data remains visible and refreshes independently. Feature prose, phase titles, progress bars, repeated group labels, and wrapped actions stay out of the card. The optional workflow-mode code and `⚙︎N` badge are reserved before title truncation.

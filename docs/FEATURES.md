...
Every lane and `OTHER ACTIVE` nests cards under the existing top-level parent group, with parent-card counts on lane and group headings. Compact density keeps every card to one row. Board subagent trees start collapsed: parents with descendants show `▸`, `Space` expands them to `▾`, and revealed children remain directly nested and independently navigable. Collapsing a selected child returns selection to its top-level parent. Active filters temporarily reveal matching ancestor/descendant context without changing expansion state. Top-level parent rows/cards add an independent `⚙︎N` when descendants are `starting` or `running`; waiting, idle, error, and stopped descendants do not count. Disclosure and the badge add no persistent state or parent-state promotion. Groups view remains expanded, and `K`/`J` reordering remains groups-view only.

There is no board feature flag. Install the [`workflow-runtime` extension from the `rules` package](https://github.com/masta-g3/rules#setup), run `/reload` in an existing Pi session if needed, invoke a tracked workflow skill, and press `S`. Rich card context is optional and appears when an extension such as `pi-session-summary` publishes the generic plan metadata described in [Configuration](CONFIG.md#session-metadata).

In all-session card density, each existing Hub group owns one junction rail in project and stage grouping. Active main sessions adapt from one to four lines: stored Hub title, optional full ticket id, optional one-line `plan.feature` description, and optional workflow/plan progress plus semantic status. The intended hierarchy uses a 1–3-word Hub title and a 5–7-word authored ticket title for `plan.feature`; the full ticket description remains separate. A feature line that repeats the stored title is omitted. Missing fields collapse without blank placeholders. `├`/`└` mark session starts; selection changes them to heavy `┣`/`┗` and highlights the full visible card. Only title lines are clickable. Backlog and subagents stay one line on their group rail, while Archived stays flat, compact, and chronological. Under height pressure, Hub always keeps the selected title. As capacity permits, it retains the rest of the selected card and its lifecycle/group headings, then uses nearby title rows before count indicators. Spare rows restore metadata only with its owning title, so clipped cards never show orphaned continuation lines.

Attention is a board-only overlay for every waiting/idle Active row, including `OTHER ACTIVE`: `✓` means a ready handoff, `?` an explicit question/choice, and `!` a blocker. Running/error/stopped rows keep operational presentation, and subagent attention is never promoted to its parent. The right pane's `work` block retains the full published title, phase title, task fraction, attention, and deterministic action once; distinct semantic metadata and live preview follow. Workflow step, Hub runtime status, attention, and running-subagent count remain independent axes: the board never infers attention from waiting, promotes child state, advances workflow, dispatches skills, moves stages, or persists board state.

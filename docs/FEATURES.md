...
The session list is sectioned as Active, Backlog, then Archived when any non-active rows exist. Active and Backlog keep group headers and manual order; Archived is one flat globally newest-first list. It shows the newest five parent cascades by default and keeps nested rows in their parent's cascade; tree disclosure still controls child visibility. Select `… N older archived` and press `Enter` or double-click to expand; use `⌃ show fewer` to collapse. Filtering reveals all matching archived rows regardless of collapse state. All-active dashboards omit the Active section header to stay compact.
